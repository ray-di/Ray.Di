...
|---|------|--------|--------------------------------|
| 1 | 束縛のキー | 型 + 修飾子。PSR-11 は実装しない | 文字列サービス ID(PSR-11 `get($id)`) |
...
## 1. 束縛のキー — 型と修飾子、そして PSR-11 を実装しない理由

ほとんどの PHP コンテナのエントリポイントは PSR-11 です。

```php
public function get(string $id): mixed;
```

キーは文字列 ID、戻り値は `mixed`。PHP-DI・Symfony・Laravel はいずれもこのインターフェースを実装しています(クラス名を ID に使う慣習はあっても、契約上は文字列です)。

Ray.Di のエントリポイントは別の形をしています。
...
PSR-11 を実装しないのは欠落ではなく設計判断です。`InjectorInterface` の phpdoc は「この舞台裏の動作こそが DI をその親戚であるサービスロケータパターンと区別するものだ」と述べています。`get($id)` を公開すると、アプリケーションコードがコンテナを持ち回って必要なときに取り出す使い方(サービスロケータ)を招きます。Ray.Di の `getInstance()` は composition root でルートオブジェクトを 1 回取得するための API で、オブジェクトグラフの内側で呼ぶことは想定されていません。
...
この判断の代償(PSR-11 前提のライブラリと直結できないこと)は 9 節で扱います。
...
**PSR-11 相互運用。** `get($id)` を前提にするライブラリ(PSR-11 を要求するミドルウェアスタックやブリッジ)とは直結できません。`getInstance()` を包む薄いアダプタは書けますが、修飾子付きのキーは文字列 ID への写像で失われます。コンテナを差し替え可能な部品として扱うエコシステムに参加することが要件なら、PSR-11 ネイティブな PHP-DI や League/Container のほうが素直です。
...
**フレームワーク統合。** Symfony / Laravel のコンテナは、自フレームワークのイベント・コンソール・キュー・設定と結線済みで、その世界の中では設定ゼロで機能します。Ray.Di はフレームワーク非依存で、深い統合は BEAR.Sunday が担います。既に Symfony / Laravel の上にいるなら、そのコンテナを使うのが摩擦最小です。
...
- PSR-11 での相互運用が中心にある、設定を非コードで管理したい、コンパイル時検証を最優先にしたい、既存フレームワークのエコシステムの内側にいる — この優先順位なら Symfony DI・PHP-DI・各フレームワークのコンテナの判断が合理的です。
